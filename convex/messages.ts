...
// helper functions for Messeges GET API:
...
export const getById = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Prevent non member from seeing it
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const messageId = await ctx.db.insert("messages", {
      ...
      // updatedAt removed because convex has its own filed which we use for comparison, If updatedAt is unddefined , this will mean original message
      // updatedAt: Date.now(),
    });
